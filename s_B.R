...
base2$q<- base$Rendimiento / (1- 0.25)
...
for(j in 1:nrow(base2)){
  
  Suma<-0
  VP<-0
  
  for(i in 1:base2$PAGOS.EXACTOS[j]){
...
base2$v<-Iteracion+ValorPresente


## Sacando u(t)

base2$u<- Iteracion
base2$U<- ValorPresente

## e(t) se supondrá como 10, ya que es un pago acumulado, es decir e(t) = 10

### El spread ####

base$Spread<- (((1-0.25*base2$Intereses.Devengados*0.25) * base2$q * base2$v * (base2$Dias.por.Vencer/360)) / 
                 (base2$q)*(base2$u+10)* (base2$Dias.por.Vencer/360) + base2$p*base2$U)/100
